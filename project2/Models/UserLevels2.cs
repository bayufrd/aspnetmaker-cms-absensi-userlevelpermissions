...
public partial class project2 {
    ...
    /// </summary>
    [MaybeNull]
    public static UserLevels2 userLevels2
    {
        get => HttpData.Resolve<UserLevels2>("user_levels2");
        set => HttpData["user_levels2"] = value;
    }

    /// <summary>
    /// Table class for user_levels
    /// </summary>
    public class UserLevels2 : DbTable
    {
        ...
        public readonly DbField<MySqlDbType> level_name;
        ...
        public UserLevels2()
        {
            ...
            TableVar = "user_levels2";
            Name = "user_levels";
            Type = "TABLE";
            UpdateTable = "user_levels"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            user_level_id = new(this, "x_user_level_id", 3, MySqlDbType.Int32) {
                ...
                CustomMessage = Language.FieldPhrase("user_levels2", "user_level_id", "CustomMsg"),
                IsUpload = false
            };
            ...
            // level_name
            level_name = new(this, "x_level_name", 200, MySqlDbType.VarString) {
                Name = "level_name",
                Expression = "`level_name`",
                UseBasicSearch = true,
                BasicSearchExpression = "`level_name`",
                DateTimeFormat = -1,
                VirtualExpression = "`level_name`",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                CustomMessage = Language.FieldPhrase("user_levels2", "level_name", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("level_name", level_name);

            // description
            description = new(this, "x_description", 200, MySqlDbType.Text) {
                Name = "description",
                ...
                CustomMessage = Language.FieldPhrase("user_levels2", "description", "CustomMsg"),
                IsUpload = false
            };
        ...
        public async Task<int> InsertAsync(object data)
        {
            ...
            try {
                ...
                if (row.ContainsKey("user_level_id"))
                    row["user_level_id"] = lastInsertedId;
                else
                    row.Add("user_level_id", lastInsertedId);
                user_level_id.SetDbValue(lastInsertedId);
                result = 1;
            } catch (Exception e) {
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            ...
            try {
                ...
                level_name.DbValue = row["level_name"]; // Set DB value only
        ...
        private string _sqlKeyFilter => "`user_level_id` = @user_level_id@";
        ...
        public string GetModalCaption(string pageName)
        {
            return pageName switch {
                "UserLevels2View" => Language.Phrase("View"),
                "UserLevels2Edit" => Language.Phrase("Edit"),
                "UserLevels2Add" => Language.Phrase("Add"),
                _ => String.Empty
            };
        }
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "UserLevels2View",
                Config.ApiAddAction => "UserLevels2Add",
                Config.ApiEditAction => "UserLevels2Edit",
                Config.ApiDeleteAction => "UserLevels2Delete",
                Config.ApiListAction => "UserLevels2List",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "UserLevels2List";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("UserLevels2View", parm);
            else
                url = KeyUrl("UserLevels2View", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "UserLevels2Add";
        ...
        {
            string url = "";
            url = KeyUrl("UserLevels2Edit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("UserLevels2List", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("UserLevels2Add", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("UserLevels2List", "action=copy"))); // DN
        ...
        public string GetDeleteUrl(string parm = "")
        {
            return UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
                ...
                : AppPath(KeyUrl("UserLevels2Delete", parm)); // DN
        }
        ...
        {
            string json = "";
            json += "\"user_level_id\":" + ConvertToJson(user_level_id.CurrentValue, "number", true);
            json = "{" + json + "}";
            if (htmlEncode)
        ...
        {
            if (!IsNull(user_level_id.CurrentValue)) {
                url += "/" + user_level_id.CurrentValue;
            } else {
        ...
        public List<string> GetRecordKeys()
        {
            ...
            if (Post("key_m[]", out sv) || Get("key_m[]", out sv)) { // DN
                ...
                if (RouteValues.TryGetValue("user_level_id", out object? v0)) { // user_level_id // DN
                    key = UrlDecode(v0); // DN
                } else if (IsApi() && !Empty(keyValues)) {
                    key = keyValues[0];
                } else {
                    key = Param("user_level_id");
                }
                keysList.Add(key);
            }
            // Check keys
            foreach (var keys in keysList) {
                if (KeyFields.Count > 1 && (!IsArray(keys) || keys.Length != KeyFields.Count))
                    continue;
                if (!IsNumeric(keys)) // user_level_id
                    continue;
                result.Add(keys);
            }
            return result;
        ...
        public string GetFilterFromRecordKeys(bool setCurrent = true)
        {
            ...
            foreach (var keys in recordKeys) {
                if (!Empty(keyFilter))
                    keyFilter += " OR ";
                if (setCurrent)
                    user_level_id.CurrentValue = keys;
                else
                    user_level_id.OldValue = keys;
        ...
        public void LoadListRowValues(Dictionary<string, object>? row)
        {
            ...
            level_name.SetDbValue(row["level_name"]);
        ...
        {
            string pageName = "UserLevels2List";
        ...
        public async Task RenderListRow()
        {
            ...
            // user_level_id
            user_level_id.ViewValue = user_level_id.CurrentValue;
            user_level_id.ViewCustomAttributes = "";

            // level_name
            level_name.ViewValue = ConvertToString(level_name.CurrentValue); // DN
            level_name.ViewCustomAttributes = "";
            ...
            // level_name
            level_name.HrefValue = "";
            level_name.TooltipValue = "";
        ...
        public async Task RenderEditRow()
        {
            ...
            user_level_id.EditValue = user_level_id.CurrentValue;
            user_level_id.ViewCustomAttributes = "";

            // level_name
            level_name.SetupEditAttributes();
            if (!level_name.Raw)
                level_name.CurrentValue = HtmlDecode(level_name.CurrentValue);
            level_name.EditValue = level_name.CurrentValue;
            level_name.PlaceHolder = RemoveHtml(level_name.Caption);
                ...
                if (recCnt >= startRec) {
                    ...
                    if (!doc.ExportCustom) {
                        ...
                        if (exportType == "view") {
                            ...
                            await doc.ExportField(level_name);
