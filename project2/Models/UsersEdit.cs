    ...
    public class UsersEditBase : Users
    {
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("user_id", out rv)) { // DN
                    user_id.FormValue = UrlDecode(rv); // DN
                    user_id.OldValue = user_id.FormValue;
                } else if (CurrentForm.HasValue("x_user_id")) {
                    user_id.FormValue = CurrentForm.GetValue("x_user_id");
                    user_id.OldValue = user_id.FormValue;
                } else if (!Empty(keyValues)) {
                    user_id.OldValue = ConvertToString(keyValues[0]);
                } else {
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("user_id", out rv)) { // DN
                        ...
                        user_id.CurrentValue = DbNullValue;
                    }
                }
        ...
        if (postBack) {
            ...
            if (IsApi() && RouteValues.TryGetValue("key", out object? k)) {
                ...
                user_id.FormValue = ConvertToString(keyValues[0]);
            }
        }
        ...
        protected Dictionary<string, object> GetEditRow(Dictionary<string, object> rsold)
        {
            ...
            // user_level_id
            if (Security.CanAdmin) { // System admin
            user_level_id.SetDbValue(rsnew, user_level_id.CurrentValue, user_level_id.ReadOnly);
            }
            return rsnew;
        }
        ...
        protected void RestoreEditFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("user_level_id", out value)) // user_level_id
                user_level_id.CurrentValue = value;
        }
