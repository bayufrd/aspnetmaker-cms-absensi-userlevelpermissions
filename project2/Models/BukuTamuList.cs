...
public partial class project2 {
    ...
    public class BukuTamuListBase : BukuTamu
    {
        ...
        public string FormName = "fbuku_tamulist";
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(nama.AdvancedSearch.ToJson())); // Field nama
            filters.Merge(JObject.Parse(umur.AdvancedSearch.ToJson())); // Field umur
            filters.Merge(JObject.Parse(tanggal_kunjungan.AdvancedSearch.ToJson())); // Field tanggal_kunjungan
            filters.Merge(JObject.Parse(tujuan.AdvancedSearch.ToJson())); // Field tujuan
            filters.Merge(JObject.Parse(komentar.AdvancedSearch.ToJson())); // Field komentar
        ...
        protected bool RestoreFilterList() {
            ...
            // Field nama
            if (filter?.TryGetValue("x_nama", out sv) ?? false) {
                nama.AdvancedSearch.SearchValue = sv;
                nama.AdvancedSearch.SearchOperator = filter["z_nama"];
                nama.AdvancedSearch.SearchCondition = filter["v_nama"];
                nama.AdvancedSearch.SearchValue2 = filter["y_nama"];
                nama.AdvancedSearch.SearchOperator2 = filter["w_nama"];
                nama.AdvancedSearch.Save();
            }

            // Field umur
            if (filter?.TryGetValue("x_umur", out sv) ?? false) {
                umur.AdvancedSearch.SearchValue = sv;
                umur.AdvancedSearch.SearchOperator = filter["z_umur"];
                umur.AdvancedSearch.SearchCondition = filter["v_umur"];
                umur.AdvancedSearch.SearchValue2 = filter["y_umur"];
                umur.AdvancedSearch.SearchOperator2 = filter["w_umur"];
                umur.AdvancedSearch.Save();
            }

            // Field tanggal_kunjungan
            if (filter?.TryGetValue("x_tanggal_kunjungan", out sv) ?? false) {
                tanggal_kunjungan.AdvancedSearch.SearchValue = sv;
                tanggal_kunjungan.AdvancedSearch.SearchOperator = filter["z_tanggal_kunjungan"];
                tanggal_kunjungan.AdvancedSearch.SearchCondition = filter["v_tanggal_kunjungan"];
                tanggal_kunjungan.AdvancedSearch.SearchValue2 = filter["y_tanggal_kunjungan"];
                tanggal_kunjungan.AdvancedSearch.SearchOperator2 = filter["w_tanggal_kunjungan"];
                tanggal_kunjungan.AdvancedSearch.Save();
            }
            ...
            // Field komentar
            if (filter?.TryGetValue("x_komentar", out sv) ?? false) {
                komentar.AdvancedSearch.SearchValue = sv;
                komentar.AdvancedSearch.SearchOperator = filter["z_komentar"];
                komentar.AdvancedSearch.SearchCondition = filter["v_komentar"];
                komentar.AdvancedSearch.SearchValue2 = filter["y_komentar"];
                komentar.AdvancedSearch.SearchOperator2 = filter["w_komentar"];
                komentar.AdvancedSearch.Save();
            }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(id); // id
                UpdateSort(nama); // nama
                UpdateSort(umur); // umur
                UpdateSort(tanggal_kunjungan); // tanggal_kunjungan
                UpdateSort(tujuan); // tujuan
                UpdateSort(komentar); // komentar
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""buku_tamu"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""buku_tamu"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""buku_tamu"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fbuku_tamulist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fbuku_tamulist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""buku_tamu"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("umur")); // DN
                CreateColumnOption(option.Add("tanggal_kunjungan")); // DN
                CreateColumnOption(option.Add("tujuan")); // DN
                CreateColumnOption(option.Add("komentar")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fbuku_tamusrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fbuku_tamusrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fbuku_tamulist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("id", "r" + ConvertToString(bukuTamuList.RowCount) + "_buku_tamu");
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fbuku_tamusrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
