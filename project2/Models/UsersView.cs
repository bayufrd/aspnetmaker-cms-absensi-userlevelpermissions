    ...
    public class UsersViewBase : Users
    {
        ...
        public UsersViewBase()
        {
            ...
            if (IsApi()) {
                ...
                if (keys.Length > 0)
                    RecordKeys["user_id"] = keys[0];
            } else {
                RecordKeys["user_id"] = RouteValues.TryGetValue("user_id", out obj) && obj != null ? UrlDecode(obj) : Get("user_id"); // DN
            }
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (RouteValues.TryGetValue("user_id", out v) && !Empty(v)) { // DN
                user_id.QueryValue = UrlDecode(v); // DN
                RecordKeys["user_id"] = user_id.QueryValue;
            } else if (Get("user_id", out sv)) {
                user_id.QueryValue = sv.ToString();
                RecordKeys["user_id"] = user_id.QueryValue;
            } else if (IsApi() && !Empty(keyValues)) {
                user_id.QueryValue = ConvertToString(keyValues[0]);
                RecordKeys["user_id"] = user_id.QueryValue;
