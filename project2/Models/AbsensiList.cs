...
public partial class project2 {
    ...
    /// absensiList
    /// </summary>
    public static AbsensiList absensiList
    {
        get => HttpData.Get<AbsensiList>("absensiList")!;
        set => HttpData["absensiList"] = value;
    }

    /// <summary>
    /// Page class for absensi
    /// </summary>
    public class AbsensiList : AbsensiListBase
    {
        // Constructor
        public AbsensiList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public AbsensiList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class AbsensiListBase : Absensi
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "absensiList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fabsensilist";
        ...
        public AbsensiListBase()
        {
            ...
            MultiDeleteUrl = "AbsensiDelete";
            MultiUpdateUrl = "AbsensiUpdate";
        ...
        // Page name
        public string PageName => "AbsensiList";
        ...
        // Constructor
        public AbsensiListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(user_id.AdvancedSearch.ToJson())); // Field user_id
            filters.Merge(JObject.Parse(tanggal.AdvancedSearch.ToJson())); // Field tanggal
            filters.Merge(JObject.Parse(jam_masuk.AdvancedSearch.ToJson())); // Field jam_masuk
            filters.Merge(JObject.Parse(jam_keluar.AdvancedSearch.ToJson())); // Field jam_keluar
            ...
            filters.Merge(JObject.Parse(foto_verifikasi.AdvancedSearch.ToJson())); // Field foto_verifikasi
        ...
        protected bool RestoreFilterList() {
            ...
            // Field user_id
            if (filter?.TryGetValue("x_user_id", out sv) ?? false) {
                user_id.AdvancedSearch.SearchValue = sv;
                user_id.AdvancedSearch.SearchOperator = filter["z_user_id"];
                user_id.AdvancedSearch.SearchCondition = filter["v_user_id"];
                user_id.AdvancedSearch.SearchValue2 = filter["y_user_id"];
                user_id.AdvancedSearch.SearchOperator2 = filter["w_user_id"];
                user_id.AdvancedSearch.Save();
            }
            ...
            // Field jam_masuk
            if (filter?.TryGetValue("x_jam_masuk", out sv) ?? false) {
                jam_masuk.AdvancedSearch.SearchValue = sv;
                jam_masuk.AdvancedSearch.SearchOperator = filter["z_jam_masuk"];
                jam_masuk.AdvancedSearch.SearchCondition = filter["v_jam_masuk"];
                jam_masuk.AdvancedSearch.SearchValue2 = filter["y_jam_masuk"];
                jam_masuk.AdvancedSearch.SearchOperator2 = filter["w_jam_masuk"];
                jam_masuk.AdvancedSearch.Save();
            }

            // Field jam_keluar
            if (filter?.TryGetValue("x_jam_keluar", out sv) ?? false) {
                jam_keluar.AdvancedSearch.SearchValue = sv;
                jam_keluar.AdvancedSearch.SearchOperator = filter["z_jam_keluar"];
                jam_keluar.AdvancedSearch.SearchCondition = filter["v_jam_keluar"];
                jam_keluar.AdvancedSearch.SearchValue2 = filter["y_jam_keluar"];
                jam_keluar.AdvancedSearch.SearchOperator2 = filter["w_jam_keluar"];
                jam_keluar.AdvancedSearch.Save();
            }
            ...
            if (filter?.TryGetValue("x_foto_verifikasi", out sv) ?? false) {
                foto_verifikasi.AdvancedSearch.SearchValue = sv;
                foto_verifikasi.AdvancedSearch.SearchOperator = filter["z_foto_verifikasi"];
                foto_verifikasi.AdvancedSearch.SearchCondition = filter["v_foto_verifikasi"];
                foto_verifikasi.AdvancedSearch.SearchValue2 = filter["y_foto_verifikasi"];
                foto_verifikasi.AdvancedSearch.SearchOperator2 = filter["w_foto_verifikasi"];
                foto_verifikasi.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(foto_verifikasi);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(tanggal); // tanggal
                UpdateSort(jam_masuk); // jam_masuk
                UpdateSort(jam_keluar); // jam_keluar
                UpdateSort(status); // status
                UpdateSort(lokasi); // lokasi
                UpdateSort(foto_verifikasi); // foto_verifikasi
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""absensi"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""absensi"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""absensi"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fabsensilist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fabsensilist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""absensi"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("user_id")); // DN
                CreateColumnOption(option.Add("tanggal")); // DN
                CreateColumnOption(option.Add("jam_masuk")); // DN
                CreateColumnOption(option.Add("jam_keluar")); // DN
                CreateColumnOption(option.Add("status")); // DN
                CreateColumnOption(option.Add("lokasi")); // DN
                CreateColumnOption(option.Add("foto_verifikasi")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fabsensisrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fabsensisrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fabsensilist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(absensiList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(absensiList.RowCount) + "_absensi");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(absensiList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && absensiList.RowType == RowType.Add || IsEdit && absensiList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // jam_masuk
                jam_masuk.HrefValue = "";
                jam_masuk.TooltipValue = "";

                // jam_keluar
                jam_keluar.HrefValue = "";
                jam_keluar.TooltipValue = "";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fabsensisrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
